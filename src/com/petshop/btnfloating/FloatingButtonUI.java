package com.petshop.btnfloating;
...
public class FloatingButtonUI extends LayerUI<JComponent> {
    ...
    public FloatingButtonUI() {
        image = new ImageIcon(getClass().getResource("/com/petshop/icon/plus.png")).getImage();
    }
    ...
    protected void processMouseEvent(MouseEvent me, JLayer<? extends JComponent> jlayer) {
        ...
        if (SwingUtilities.isLeftMouseButton(me)) {
            if (me.getID() == MouseEvent.MOUSE_PRESSED) {
                ...
                jlayer.repaint(shape.getBounds());
                if (mouseHovered) {
                    JComponent main = jlayer.getView();
                    if (main instanceof Dashboard) {
                        ((Dashboard) main).onFloatingButtonClick();
                    }
                }
            }
        }
